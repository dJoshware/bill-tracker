...
export default function HomePage() {
  ...
  const visibleBills = useMemo(() => {
    let filtered = bills.filter((b) => b.recurring || b.monthKey === monthKey);
    if (filterCat !== "all") filtered = filtered.filter((b) => b.category === filterCat);
  ...
  const notifStatus = typeof window !== "undefined" ? getPermissionStatus() : "default";
    ...
    <div style={{ maxWidth: 480, margin: "0 auto", paddingBottom: 40 }}>
      ...
      >
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 18 }}>
          <div>
            <div style={{ fontFamily: "var(--font-dm-serif)", fontSize: 28, color: "var(--accent)", lineHeight: 1 }}>
              Billfold
            </div>
            <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 4, letterSpacing: "0.04em", textTransform: "uppercase" }}>
              Personal Finance
            </div>
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <div style={{ fontSize: 13, fontWeight: 500, minWidth: 80, textAlign: "center" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10, marginBottom: 14 }}>
          {[
            { label: "Total", value: formatCurrency(total), color: "var(--text)" },
            { label: "Paid", value: formatCurrency(paidTotal), color: "var(--success)" },
            { label: "Left", value: formatCurrency(remaining), color: remaining > 0 ? "var(--danger)" : "var(--success)" },
          ].map((s) => (
            <div
              key={s.label}
              style={{
      ...
      <div style={{ padding: "20px 16px" }}>
        ...
        {visibleBills.length === 0 ? (
          <div style={{ textAlign: "center", padding: "56px 20px" }}>
            ...
            <div style={{ fontSize: 16, color: "var(--muted)", lineHeight: 1.6 }}>
              {bills.length === 0
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {visibleBills.map((bill) => (
              <BillCard
                ...
                onTogglePaid={(id) => {
                  togglePaid(id);
                  showToast(paid[id] ? "Marked unpaid" : "Marked as paid! ✓");
                }}
                onEdit={handleEdit}
                onDelete={(id) => {
                  deleteBill(id);
        ...
        {visibleBills.length > 0 && (
          ...
          >
            ...
            <span>
              ...
              {visibleBills.filter((b) => !paid[b.id]).length} remaining
            </span>
          </div>
        )}
      </div>
      ...
      {showNotif && (
        <NotificationPanel bills={bills} onClose={() => setShowNotif(false)} />
      )}
